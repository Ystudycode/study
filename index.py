...
# User = 'oNcUE6Y-TFW4FuSssf6RPij8IO1M' #微信测试平台用户列表微信号
...
def message():  ##信息模板
    return {
        ...
        "data": {
            ...
            "word1": { ## 时间+星期几
                "value": str(datetime.datetime.now().strftime('%Y-%m-%d'))+' '+week_list[datetime.date(2022, 2, 22).weekday()],
                "color": randomcolor()
            },
            "citydata": { ## 城市天气状态
...
json_data = {"标题":mss,"时间":str(datetime.datetime.now().strftime('%Y-%m-%d'))+' '+week_list[datetime.date(2022, 2, 22).weekday()],
